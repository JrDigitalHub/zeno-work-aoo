...
func (d *DiscoveryAgent) ExtractLeads(ctx context.Context, workspaceID string, query string, mode string) {
	if d.apiKey == "" {
	...
	switch mode {
	case "SOCIAL_HUNTER":
		d.runSocialHunter(ctx, workspaceID, query)
	case "DEEP_CRAWLER":
		d.runDeepCrawler(ctx, workspaceID, query)
	case "LOCAL_SCANNER":
		fallthrough
	default:
		d.runLocalScanner(ctx, workspaceID, query)
	}
}
...
// ---------------------------------------------------------
func (d *DiscoveryAgent) runLocalScanner(ctx context.Context, workspaceID, query string) {
	fmt.Printf("📍 [DISCOVERY] Local Grid Scanner Engaged. Target: \"%s\"\n", query)

	payload := map[string]interface{}{"q": query}
	jsonPayload, _ := json.Marshal(payload)

	req, err := http.NewRequestWithContext(ctx, "POST", "https://google.serper.dev/places", bytes.NewBuffer(jsonPayload))
	if err != nil {
		fmt.Printf("❌ [DISCOVERY] Failed to create HTTP request: %v\n", err)
		return
	}
	...
	for _, hit := range result.Places {
		if hit.Website != "" && strings.HasPrefix(hit.Website, "http") {
			...
			err = d.router.Publish(ctx, protocol.Event{
			...
			if err != nil {
				fmt.Printf("❌ [DISCOVERY] Failed to publish local target: %v\n", err)
			}
			count++
			// Use context-aware sleep or respect context cancellation during sleep
			select {
			case <-ctx.Done():
				return
...
// ---------------------------------------------------------
func (d *DiscoveryAgent) runSocialHunter(ctx context.Context, workspaceID, query string) {
	...
	req, err := http.NewRequestWithContext(ctx, "POST", "https://google.serper.dev/search", bytes.NewBuffer(jsonPayload))
	if err != nil {
		fmt.Printf("❌ [DISCOVERY] Failed to create HTTP request: %v\n", err)
		return
	}
	...
	for _, hit := range result.Organic {
		if strings.Contains(hit.Link, "linkedin.com/in/") {
			...
			if err != nil {
				fmt.Printf("❌ [DISCOVERY] Failed to publish social target: %v\n", err)
			}
			count++
			select {
			case <-ctx.Done():
...
// ---------------------------------------------------------
func (d *DiscoveryAgent) runDeepCrawler(ctx context.Context, workspaceID, query string) {
		...
		if link != "" && strings.HasPrefix(link, "http") {
			...
			if !strings.Contains(lowerLink, "ycombinator.com") &&
				...
				if !uniqueTargets[link] {
					...
					if err != nil {
						fmt.Printf("❌ [DISCOVERY] Failed to publish crawl target: %v\n", err)
					}
					count++
					select {
					case <-ctx.Done():
